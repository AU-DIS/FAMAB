                                    ...
                                    const std::string &out_path = "/tmp/out")
{
    ...
    for (int i = 0; i < averages; i++)
    {
        ...
        std::thread t3(basic_tsallis_runner<QBL>, std::ref(fpl_second), std::ref(data_matrix), rounds, std::ref(fpl_second_run));

        std::vector<double> uniform_run;
        std::thread t4(basic_tsallis_runner<Uniformbandit>, std::ref(uni), std::ref(data_matrix), rounds,
                       std::ref(uniform_run));
