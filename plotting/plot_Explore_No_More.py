...
fig = plt.figure()
ax = fig.add_subplot()
ax.errorbar(df.loc[:,'eta'], df.loc[:,'mean_regret_exp3_T_half'], df.loc[:,'std_regret_exp3_T_half'], label = "mean regret Exp3")
ax.errorbar(df.loc[:,'eta'], df.loc[:,'mean_regret_exp3IX_T_half'], df.loc[:,'std_regret_exp3IX_T_half'], label = "mean regret Exp3IX")
ax.set_xscale('log')
ax.legend()
plt.show()